...
def run_benchmarks():
    ...
    plt.plot(sizes, dnc_hull_times, c='blue', label='DNC Hull Times')
    plt.scatter(sizes, naive_hull_times, c='red')
    plt.plot(sizes, naive_hull_times, c='red', label='Naive Hull Times')
    plt.legend()
    plt.xlabel('number of points (n)')
    plt.ylabel('Time')
    plt.title('Convex Hull Benchmarking')
